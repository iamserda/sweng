"""Challenge:
Given an array of integers, find two elements within the array
where their product is some integer K.
[2,40,1,6,5,4,-1] -> K=20 => 5, 4 since 5 x 4 = 20
"""


def solution(arr: list[int], k: int) -> tuple:
    setti = set()
    for num in arr:
        complement = k // num
        if k % num == 0 and complement in setti:
            return num, complement
        setti.add(num)
    return tuple()


# print(solution([3, 6, 2, 15, 9, 7, 5, 10], k=30))
assert solution([3, 6, 2, 15, 9, 7, 5, 10], k=30) == (2, 15) or (15, 2)
assert solution([3, 2, 15, 9, 7, 5, 10], k=45) == (3, 15) or (15, 3) or (9, 5) or (5, 9)
